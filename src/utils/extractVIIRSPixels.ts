...
export function extractVIIRSPixelsFromGeoJSON(
  ...
  featureCollection.features.forEach((feature: Feature, index: number) => {
    ...
    if ((index + 1) % 100 === 0) {
      console.log(`Processed ${index + 1} features, found ${allUniquePixels.size} unique pixels so far`);
    }
  });
  ...
  console.log(`Pixels distributed across ${Object.keys(pixelsByTile).length} tiles:`);
